...
These variables capture how effectively a team transformed possession and shots into goals. For each match, we compare the two teams on each efficiency variable. Each difference is first divided by the standard deviation of that variable, so that variables with larger raw values do not dominate the score. We then average those normalized differences:

\[
    E(A,B) = \frac{1}{|F_E|}\sum_{f \in F_E}
    \frac{x_f(A)-x_f(B)}{\sigma_f}.
\]

Here, $F_E$ is the set of efficiency features and $|F_E|$ is the number of features in that set. A positive value means that team $A$ was more efficient than team $B$ in that match. The edge is directed from the more efficient team to the less efficient opponent, with weight $|E(A,B)|$.
...
For every match, we compare the two teams on these two variables. As before, each difference is first divided by the standard deviation of that variable, and the normalized differences are then averaged:

\[
    R(A,B) = \frac{1}{|F_R|}\sum_{f \in F_R}
    \frac{x_f(A)-x_f(B)}{\sigma_f}.
\]

Here, $F_R$ contains defensive pressures applied and forced turnovers, so $|F_R|=2$. A positive value means that team $A$ had the stronger pressure/recovery profile in that match. If $R(A,B)>0$, the edge points from $A$ to $B$; otherwise, it points from $B$ to $A$. This network highlights teams that consistently imposed defensive pressure and recovered possession.
